...
## Часть 3 - cgroups
Небольшое предупреждение. В 3 пункте я использовала $CG, которое магическим образом в будущем в отчете поменяется на $CGROUP. Неточности выполнения отчета разными людьми, но суть у этих обозначений одна и та же.
Для начала с помощью команды echo 100M | sudo tee "$CGROUP/memory.max" я задала ограничение на использование памяти контейнера, а также запретила командой echo 0    | sudo tee "$CGROUP/memory.swap.max" выгружать на диск занятое контейнером пространство. при вводе /eat?mb=200 произошло то самое oom_kill, которое показано на изображении.
![alt text](cashe/image_3_1.png)

Также процесс в соседнем терминале был убит из-за переполнения памяти, что показано на данном скриншоте.
![alt text](cashe/image_3_2.png)

Дальше я снова подняла процесс, и задала для него ограничение echo "50000 100000" | sudo tee "$CG/cpu.max", которое означает что на каждые 100мс разрешена работа контейнера в течение 50мс. 
Состояние системы до её нагрузки с помощью /burn выглядит так (стоит обратить внимание на строчки nr_throttled = 0 и throttled_usec = 0):
![alt text](cashe/image_3_3.png)

Далее я запустила /burn и подождала в районе 10-15 секунд. Запустила команду top, которая в реальном времени показывает влияние процесса на систему, и увидела что CPU занят ровно на 50%. Это показано на изображении ниже:
![alt text](cashe/image_3_4.png)

Снова запустила команду cat, проверить как поменялось состояние процесса. Все значительно изменилось, теперь nr_throttled = 2908 (из 2913 периодов!) и trottled_usec = 163570910. Это также показано на изображении ниже:
![alt text](cashe/image_3_5.png)

Последний пункт. С помощью команды echo 20 | sudo tee "/sys/fs/cgroup/lab 1/pids.max" я поставила ограничение - сколько процессов могут одновременно находиться в одном cgroup. Далее командой stress-ng --fork попыталась создать количество процессов, превышающее заданный лимит. Результат можно увидеть на скриншоте с помощью команды cat - ядро отказало в создании процесса 7820 раз.
![alt text](cashe/image_3_6.png)
...
Установила runsc, с помощью которого включается gVisor. На скриншоте показано что runsc установлен и готов к работе.
![alt text](cashe/image_7_1.png)

При первой попытке запуска контейнера с gVisor возникла проблема. gVisor попытался создать свой сетевой стек. Проблема в том, что в новой версии Linux возникает ошибка, при которой gVisor путается и считает что настроенный с помощью Docker namespace является корневым. Так делать категорически нельзя, поэтому он роняет контейнер, что показано на скриншоте.
![alt text](cashe/image_7_2.png)

Немного помучала ии, он нашел схожую проблему в официальном баг-трекере (Debian), и ее решение. Это же решение я воспроизвела в своей работе. По сути я просто отключила у gVisor необходимость настраивать свои сетевые интерфейсы, заставив его работать в сквозном режиме, то есть отправлять сетевые запросы напрямую в сетевой стек хоста. Возможно так делать не совсем корректно, но иного решения проблемы я не нашла. Показываю на скрине решение проблемы.
![alt text](cashe/image_7_3.png)

После этого контейнер запустился корректно, все работало в штатном режиме, как и показано на скриншоте.
![alt text](cashe/image_7_4.png)

Данные команды показывают что контейнер в остальном работает точно также изолированно. на запрос вывода процессов у api показывается PID 1. gVisor меняет то, как обрабатываются запросы из контейнера. docker exec api-runsc dmesg | head -n 5 является показательным в вопросе того, как работает gVisor - вместо отправки запроса к самому ядру, он перехватывает запрос и обрабатывает его в пользовательском режиме. docker run -d --name api-runc -p 8082:8080 api:plain - это Docker, который был запущен без gVisor, и он как раз обращается напрямую к ядру и это доказывается тем, что видна настоящая версия ядра. На прошлом скриншоте, при запросе uname -r внутри api-runsc показал 4.4.0, то есть не настоящую версию ядра, а значение, которое разработчики заранее прописали. Основной минус запуска контейнера без gVisor - уязвимость ядра автоматически становится уязвимостью и для контейнера. Эта уязвимость остается и для контейнера с gVisor, но риск значительно снижается, за счет прослойки Sentry.
![alt text](cashe/image_7_5.png)

У контейнера с хостом общим остается ядро, на котором выполняются все программы. Это предел изоляции, потому что железо и ядро остаются общими. Если сравнивать с ВМ, в которых изоляция максимальна, то там устанавливается полностью новое ядро и выделяется виртуальное железо под его нужды. Плюс существует прослойка гипервизора, которая делит физическое железо сервера на части и дает каждой виртуальной системе её собственную память, процессор и диск. 
